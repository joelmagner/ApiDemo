...
public class MiniGramClient(HttpClient client) : IMiniGramClient
{
    ...
    public async Task<Response<User>> CreateUser(CreateUserRequest request,
        CancellationToken cancellationToken = default)
    {
    ...
    public async Task<Response<Photo[]>> GetUserPhotos(Guid userId,
        CancellationToken cancellationToken = default)
    {
        var response = await client.GetAsync($"/user/photos/{userId}", cancellationToken);
        return await response.ToClientItemResponse<Photo[]>(cancellationToken);
    }

    public async Task<Response<User>> GetUserByUsername(GetUserByUsernameRequest request,
        CancellationToken cancellationToken = default)
    {
    ...
    public async Task<Response<Photo>> UploadPhoto(UploadPhotoRequest request,
        CancellationToken cancellationToken = default)
    {
